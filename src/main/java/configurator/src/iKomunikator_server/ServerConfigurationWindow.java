	...
	*/
	...
	private void saveToXml() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "KonfiguracjÄ™ poprawnie zapisano do pliku server.cfg");
		} catch (Exception e) {
...
//		fWords.addElement("ktury");
		...
		btnZmien.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (textNewWord.getText().length() == 0) {
					JOptionPane.showMessageDialog(null, "WprowadÅº wyraÅ¼enie !");
				} else if (textNewWord.getText().length() > 50) {
					JOptionPane.showMessageDialog(null, "Maksymalna dÅ‚ugoÅ›Ä‡ wyraÅ¼enia to 50 znakÃ³w!");
				} else {
		...
		JLabel lblNewLabel = new JLabel("Lista wyraÅ¼eÅ„ zabronionych:");
